...
namespace ArcherStudio.Badge.Runtime
{
    ...
    public abstract class BadgeNotificationBase<TValue> where TValue : struct
    {
        ...
        public void AddBadge(string key, int value, NodeType nodeType = NodeType.Multiple)
        {
            ...
            for (int i = 0; i <= keySpan.Length; i++)
            {
                if (i == keySpan.Length || keySpan[i] == Const.SEPARATOR)
                {
                    ...
                    node.Value.badgeCount = ClampCount(node.Value.nodeType, node.Value.badgeCount + value);
                    _trieMap.Add(node.Path, node.Value);

                    //notify ui — publish với path của chính intermediate, không phải leaf key
        ...
        {
            if (badgeData.Path == null || _trieMap.HasKey(badgeData.Path)) return;
            ...
            for (int i = 0; i <= keySpan.Length; i++)
            {
                if (i == keySpan.Length || keySpan[i] == Const.SEPARATOR)
                {
                    ...
                    node = node.GetChild(word);
                    node.Value.badgeCount = ClampCount(node.Value.nodeType, node.Value.badgeCount + badgeData.Value.badgeCount);
                    _trieMap.Add(node.Path, node.Value);
        ...
        public void UpdateBadgeCount(string key, int delta)
        {
            ...
            int oldCount = targetNode.Value.badgeCount;
            int newCount = ClampCount(targetNode.Value.nodeType, oldCount + delta);
            if (oldCount == newCount) return;
            targetNode.Value.badgeCount = newCount;
            BadgeMessaging<TValue>.UpdateBadge(key, targetNode.Value);

            PropagateDelta(targetNode, oldCount, newCount);
        }

        /// <summary>
        /// Contribution of a node's badgeCount to its parent, based on nodeType.
        /// </summary>
        private static int Contribution(NodeType nodeType, int count)
        {
            // Single và HasBadge đều đóng góp 0/1 cho parent
            if (nodeType == NodeType.Single || nodeType == NodeType.HasBadge)
                return count > 0 ? 1 : 0;
            return count; // Multiple
        }

        /// <summary>
        /// Clamp count theo semantic của nodeType. HasBadge → 0 hoặc 1.
        /// </summary>
        private static int ClampCount(NodeType nodeType, int count)
        {
            if (nodeType == NodeType.HasBadge)
                return count > 0 ? 1 : 0;
            return count;
        }

        /// <summary>
        /// Propagate a count change of <paramref name="node"/> up the tree using delta arithmetic
        /// instead of re-summing all siblings. Breaks early once an ancestor's value is unchanged.
        /// </summary>
        private void PropagateDelta(TrieNode<BadgeData<TValue>> node, int oldCount, int newCount)
        {
            int delta = Contribution(node.Value.nodeType, newCount) - Contribution(node.Value.nodeType, oldCount);
            if (delta == 0) return;

            var parent = node.Parent;
            while (parent != null && parent.Value != null)
            {
                int oldParent = parent.Value.badgeCount;
                int newParent = oldParent + delta;
                if (newParent == oldParent) break;

                parent.Value.badgeCount = newParent;
                BadgeMessaging<TValue>.UpdateBadge(parent.Path, parent.Value);

                delta = Contribution(parent.Value.nodeType, newParent) - Contribution(parent.Value.nodeType, oldParent);
                if (delta == 0) break;
        ...
        private void UpdateNodeAndChildren(TrieNode<BadgeData<TValue>> node, int delta, Func<TrieNode<BadgeData<TValue>>, bool> condition = null)
        {
            ...
            int oldCount = node.Value.badgeCount;
            int newCount = oldCount;

            if (condition == null || condition(node))
            {
                newCount = ClampCount(node.Value.nodeType, oldCount + delta);
            }
            ...
            if (node.HasChildren())
            {
                ...
                foreach (var child in node.GetChildren())
                {
                    sum += Contribution(child.Value.nodeType, child.Value.badgeCount);
                }

                newCount = ClampCount(node.Value.nodeType, sum);
            }

            if (newCount == oldCount) return; // no-op: skip publish
            node.Value.badgeCount = newCount;
            BadgeMessaging<TValue>.UpdateBadge(node.Path, node.Value);
        }
        ...
        private void UpdateNodeAndChildren(TrieNode<BadgeData<TValue>> node, int delta, TValue value, Func<TrieNode<BadgeData<TValue>>, bool> condition = null)
        {
            ...
            if (newCount == oldCount && !valueChanged) return;
            node.Value.badgeCount = newCount;
            BadgeMessaging<TValue>.UpdateBadge(node.Path, node.Value);
        }
        ...
        private void UpdateParents(TrieNode<BadgeData<TValue>> node)
        {
            ...
            while (parent != null && parent.Value != null)
            {
                ...
                int newCount = ClampCount(parent.Value.nodeType, sum);
                if (newCount == parent.Value.badgeCount)
                {
                    // Parent không đổi → ông bà cũng không đổi theo delta logic. Break sớm.
                    break;
                }

                parent.Value.badgeCount = newCount;
                BadgeMessaging<TValue>.UpdateBadge(parent.Path, parent.Value);
        ...
        public virtual void SetBadgeCount(string key, int badgeCount)
        {
            ...
            int newCount = ClampCount(node.Value.nodeType, badgeCount);
            int oldCount = node.Value.badgeCount;
            if (oldCount == newCount) return;
            ...
            PropagateDelta(node, oldCount, newCount);
        ...
        private void SetNodeAndChildrenValue(TrieNode<BadgeData<TValue>> node, int count, TValue value, Func<TrieNode<BadgeData<TValue>>, bool> condition = null)
        {
            ...
            if (condition == null || condition(node))
            {
                newCount = ClampCount(node.Value.nodeType, count);
                if (!EqualityComparer<TValue>.Default.Equals(node.Value.value, value))
                {
                    node.Value.value = value;
                    valueChanged = true;
        ...
        private void SetNodeAndChildrenValue(TrieNode<BadgeData<TValue>> node, Action<BadgeData<TValue>> modify, Func<TrieNode<BadgeData<TValue>>, bool> condition = null)
        {
            ...
            {
                modify(node.Value);
                // Clamp lại nếu modify đẩy count vượt semantic của HasBadge
                node.Value.badgeCount = ClampCount(node.Value.nodeType, node.Value.badgeCount);
            }
            ...
            if (node.HasChildren())
            {
                ...
                node.Value.badgeCount = ClampCount(node.Value.nodeType, sum);
            }

            bool changed = node.Value.badgeCount != oldCount
                           || !EqualityComparer<TValue>.Default.Equals(node.Value.value, oldValue);
            if (!changed) return;
            BadgeMessaging<TValue>.UpdateBadge(node.Path, node.Value);
        }
